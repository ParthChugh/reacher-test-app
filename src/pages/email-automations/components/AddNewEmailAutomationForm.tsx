...
import { createEmailAutomation, getEmailAutomations } from "../../store/emailAutomations";
...
interface AddNewEmailAutomationFormProps {
  onReturn: () => void;
}
const AddNewEmailAutomationForm: React.FC<AddNewEmailAutomationFormProps> = ({
  onReturn,
}) => {
  ...
  const handleNext = (values: FormikValues) => {
    ...
    if (values.brand_image instanceof File) {
      setBrandImageFile(values.brand_image);
    }
    if (values.product_image instanceof File) {
      setProductImageFile(values.product_image);
    }
    setStep(step + 1);
  };
...
const handleLoginSubmit = async (values, setSubmitting) => {
    try {
      ...
      const payload = {
        ...
        brand_image_url: brandImageData ? brandImageData[0] : null,
        brand_image_name: brandImageData ? brandImageData[1] : null,
        product_image_url: productImageData ? productImageData[0] : null,
        product_image_name: productImageData ? productImageData[1] : null,
        filters: jsonConfig,
        signature: values.signature,
        signoff: values.signoff,
        headline: values.headline,
        email_account_id: parseInt(values.email_account_id, 10), // Convert to integer
      };
      console.log("Payload before sending", payload);
  
      await dispatch(createEmailAutomation(payload));
...
export default AddNewEmailAutomationForm;